	...
	*/
	  ...
	  function arbitrageMarket(
		...
		assembly {
			...
			// pseudo - else
			if iszero(OneToTwo) {
				new_r0 := sub(reserve1, amountIn)
			}

			let new_r1 := div(k, new_r0)

			if OneToTwo {
				repay := div(mul(sub(new_r1, reserve1), 1000), 994)
			} 
			// pseudo - else
			if iszero(OneToTwo) {
				repay := div(mul(sub(new_r1, reserve0), 1000), 994)
			}
		}
	...
	function uniswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external {
		...
		if (amount0 == 0) {
			...
			uint256[] memory tokensOut = SushiRouter(SUSHIROUTER).swapExactTokensForTokens(amount1, 0, route, address(this), MAX_UINT);
			...
			uint256[] memory tokensOut = SushiRouter(SUSHIROUTER).swapExactTokensForTokens(amount0, 0, route, address(this), MAX_UINT);
